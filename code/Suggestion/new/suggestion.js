;;;(function (window,undefiend){
	...
	var sPrototype = {
		_init: function(){
			...
			this.setGetData(this._getData, this._formatData);
			...
			this._list = lib.extend.useComponent("ViewList",{
				events: {
					...
					mouseup: function(e, model){
						if(e.which==1&&model.getEl()===this.mouseItem){
		...
		_initInputEvent: function(input){
			...
			if(input.tagName == "INPUT"&&input.type == "text"){
				...
				this._oldEvents.currentInput.input = function(e){
					that._currentInput.value != "" ? that.getData(that._currentInput.value) : that.hide();
				}
		...
		_renderItem: function(data){
			...
			if(this._formatData){
				...
				this._list.set("format", function(data){
					...
					li.innerHTML = dataFlag.replace(reg,function($1, $2){
		...
		_resizePanel: function(){
			if(this._isShow){
				lib.dom.setStyle(this._panel,{
					left: this._currentInput.offsetLeft+(this._default.offsetLeft||0),
					top: this._currentInput.offsetTop+this._currentInput.offsetHeight+(this._default.offsetTop||0),
					width: this._currentInput.offsetWidth+(this._default.offsetWidth||0)
				});
			}
		},
